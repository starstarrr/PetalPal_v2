function getLoginElement(id) {
  ...
  async function finishLogin(user) {
    if (!user || !user.id) {
      throw new Error(
        "The server did not return a valid user"
      );
    }
    ...
    if (
      ...
      console.log(
        "Joined personal room after login:",
        `user:${user.id}`
      );
    }
    ...
    window.history.replaceState(
      {},
      "",
      `${window.location.pathname}?userId=${encodeURIComponent(
        user.id
      )}`
    );
    ...
    await Promise.all([
      renderFriendsList(),
      typeof renderFriendRequests === "function"
